...
def check(encoding, other_encoding, parameters):	#X must be a numpy array and parametes must be a dictionary containing numpy arrays
    
	dist = np.sum((other_encoding - encoding)**2, axis = 0)
	return dist

def predict(X, parameters, encodings, names_encodings, tau=0.4):
    ...
    for i in range(len(encodings)):
        dist = check(encoding, encodings[i], parameters)
        if(dist < least):
            least = dist
...
images_vi = [cv2.imread(file) for file in glob.glob("Test/Vipin/*")]
images_va = [cv2.imread(file) for file in glob.glob("Test/Vaibhav/*")]
...
for i in range(len(images_vi)):
    images_vi[i] = cv2.resize(images_vi[i], (200, 200))
    names_vi.append('Vipin')
for i in range(len(images_va)):
    images_va[i] = cv2.resize(images_va[i], (200, 200))
    names_va.append('Vaibhav')
...
images = []
images = images_a + images_d + images_vi + images_va + images_n
names = names_a + names_d + names_vi + names_va + names_n
images_reshaped = images #Defining a new list containing the reshaped numpy arrays
num_encodings = len(images_a) + len(images_d) + len(images_vi) + len(images_va)
...
images_vi = [cv2.imread(file) for file in glob.glob("Train/Vipin/*")]
images_va = [cv2.imread(file) for file in glob.glob("Train/Vaibhav/*")]
...
names_encodings = names_a + names_d + names_vi + names_va + names_n

#Main code


for i in range(len(images_reshaped)):
	name = predict(images_reshaped[i], parameters, encodings, names_encodings, 15)
